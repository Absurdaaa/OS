...
split_merge_test(void) {
    ...
    struct Page *p1 = alloc_pages(8);
    if (p1 == NULL) {
        cprintf("  INFO: Cannot allocate 8 pages, skipping test\n\n");
        return;
    }
    cprintf("  Allocated 8 pages (property=%u)\n", p1->property);
