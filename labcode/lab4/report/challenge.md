...
避免嵌套调用时错误地恢复中断状态, 保证临界区的完整性不被破坏, 支持函数的可重入性
...
首先，从实际使用来看，绝大多数情况下我们调用 `get_pte()` 时都是希望"找到就返回，找不到就创建"。比如在建立页表映射、处理缺页异常的时候，查找失败肯定要分配新的页表页，不然后面没法继续。如果拆成两个函数，反而要写成这样：

```c
pte_t *ptep = lookup_pte(pgdir, addr);
if (!ptep) {
    ptep = alloc_pte(pgdir, addr);
}
```

性能上多了一次查找开销（因为 `alloc_pte` 内部还得再走一遍查找逻辑）。现在一个 `create` 参数就解决了，简洁明了：

```c
pte_t *ptep = get_pte(pgdir, addr, 1);  // 查找+创建
pte_t *ptep = get_pte(pgdir, addr, 0);  // 仅查找
```

另外，页表查找在系统运行时非常频繁，把查找和分配写在一起能减少函数调用开销，也不用重复走查找流程，性能上有优势。
